...
export class ThemeService {
  ...
  async init({ deferNative = false } = {}) {
    ...
    await this.applyTheme(saved, { deferNative });
  ...
  async applyTheme(themeName, { deferNative = false } = {}) {
    ...
    if (!deferNative) await this.syncNativeStatusBar();
  ...
  async syncNativeStatusBar() {
    ...
    try {
      ...
      await StatusBar.setBackgroundColor({ color: colorBg });
    } catch (err) {
