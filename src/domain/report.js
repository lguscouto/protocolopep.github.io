...
export function generateReportCSV(entries = [], context = {}) {
  ...
  const BOM = "\uFEFF";
  const headers = [tx("date", "Data"), tx("time", "Hora"), tx("compound", "Peptídeo"), tx("subtitle", "Subtítulo"), tx("dose", "Dose"), tx("unitsUi", "UI"), tx("syringe", "Seringa (UI)"), tx("type", "Tipo"), tx("notes", "Observações"),
...
export function generateMeasurementsCSV(measurements = [], context = {}) {
  const tx = createReportTranslator(context);
  const headers = [tx("date", "Data"), tx("time", "Hora"), tx("weightKg", "Peso (kg)"), `${tx("energy", "Energia")} (${tx("scale15", "1–5")})`, `${tx("mood", "Humor")} (${tx("scale15", "1–5")})`, tx("symptoms", "Sintomas"), tx("intensities", "Intensidades"), tx("notes", "Observações"), tx("origin", "Origem"), tx("ownership", "Propriedade")];
...
<body>
  <div class="header">
    <div>
      ...
      <div class="meta">${escapeHTML(tx("period"))}: <b>${safePeriodText}</b> · ${escapeHTML(tx("issuedAt"))}: <b>${dStr} ${escapeHTML(tx("issuedAtSeparator", locale === "pt-BR" ? "às" : locale === "es" ? "a las" : "at"))} ${tStr}</b></div>
    </div>
  </div>
