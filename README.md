...
![Version](https://img.shields.io/badge/version-3.9.15-2CC5C0)
...
## 🚀 Correção da Versão 3.9.15

- **Adicionar tratamento em Hoje:** o botão secundário volta a aparecer quando já existe um tratamento, inclusive sem pendência no dia ou com início futuro, e abre o mesmo editor usado em Mais → Tratamento.
- **Fluxo único e acessível:** o acionamento usa delegação de eventos, preserva foco ao fechar e mantém o CTA vazio separado quando ainda não há tratamentos.
- **Release Android versionada:** `versionName 3.9.15` e `versionCode 44`. O APK é debug para avaliação; teste em dispositivo físico não foi executado por decisão de escopo.
...
- **Filtros do Histórico:** textos traduzidos de forma consistente nos três idiomas e controles de seleção redesenhados com foco visível, melhor contraste e área de toque mínima de 44 px.
- **Validação:** testes unitários, build web e E2E do Histórico executados após a correção.
...
## 🚀 Novidades da Versão 2.9.9 (Mapa de Aplicação Refinado)

- **Release Android versionada:** `versionName 2.9.9` e `versionCode 23`, com APK instalável publicado no GitHub Releases.
- **Mapa baseado na imagem de referência:** pontos 1–4 aproximados do umbigo e pontos 5–6 reposicionados nas laterais inferiores dos flancos.
- **Marcadores mais discretos:** círculos de aplicação reduzidos visualmente, preservando a área de toque mínima de 44 px, numeração, seleção e estados de rotação.
- **Validação concluída:** 329 testes unitários, 79 cenários E2E aprovados, build web/Android, lint e APK instalado no emulador.
...
## 🚀 Novidades da Versão 2.9.5 (Regressão Visual e Documentação)

- **Release Android versionada:** `versionName 2.9.5` e `versionCode 21`, com APK instalável publicado no GitHub Releases.
- **Matriz de regressão visual:** validação automatizada dos temas escuro, claro e alto contraste em viewports de 360 px, 412 px e 600 px, cobrindo dashboard, mapa de aplicação, semana, histórico, medidas, inventário, pesquisa, ajustes e onboarding.
- **Consistência responsiva:** proteções contra overflow horizontal e navegação móvel incompleta nos fluxos críticos, preservando hierarquia e legibilidade em telas compactas.
- **Referências de loja atualizadas:** screenshots reproduzíveis do dashboard, linha do tempo e acompanhamento pessoal, gerados a partir de estados sintéticos locais do app.
- **Qualidade e conformidade auditadas:** 318 testes unitários, 69 cenários E2E (incluindo auditoria WCAG 2.1 AA via Axe), build web e testes unitários Kotlin/geração do APK validados.
...
## 🚀 Novidades da Versão 2.7.0 (Release Android, Tempo Histórico Determinístico e CI Atualizado)

- **Release Android versionada:** `versionName 2.7.0` e `versionCode 18`, com APK instalável publicado no GitHub Releases.
